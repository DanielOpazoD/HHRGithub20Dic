...
test.describe('Census Closing Flow', () => {
    ...
    test('should allow admin to close current census day', async ({ page }) => {
        ...
        const printBtn = page.locator('button[title="Imprimir Censo"], button:has-text("Imprimir")').first();
        ...
        if (await printBtn.isVisible()) {
            ...
            console.log('No explicit Close button found, skipping close step but verifying daily record presence');
        }
