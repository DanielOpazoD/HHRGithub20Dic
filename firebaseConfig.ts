...
const fetchRuntimeConfig = async (timeoutMs = 5000): Promise<FirebaseOptions> => {
...
const loadFirebaseConfig = async () => {
    ...
    if (!window.navigator.onLine && cached?.apiKey) {
        console.warn('[FirebaseConfig] Offline detected — using cached Firebase config');
        return cached;
    }

    try {
        const config = await fetchRuntimeConfig(5000);
        if (config.apiKey) {
        ...
        const envFallback = buildDevConfig();
        if (envFallback.apiKey) {
            console.warn('[FirebaseConfig] Using env fallback config because runtime fetch failed');
            return envFallback;
        }
