...
/**
 * Store passport in localStorage for quick offline access.
 */
export const storePassportLocally = (passport: OfflinePassport): void => {
    try {
        localStorage.setItem('hhr_offline_passport', JSON.stringify(passport));
        console.log('[Passport] Stored passport locally for:', passport.email);
    } catch (error) {
...
/**
 * Retrieve stored passport from localStorage.
 */
export const getStoredPassport = (): OfflinePassport | null => {
    try {
        const stored = localStorage.getItem('hhr_offline_passport');
        if (!stored) return null;
        return JSON.parse(stored) as OfflinePassport;
    } catch {
        return null;
    }
};
...
/**
 * Clear stored passport from localStorage.
 */
export const clearStoredPassport = (): void => {
