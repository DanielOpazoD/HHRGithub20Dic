...
describe('dateUtils', () => {
  ...
  describe('isAdmittedDuringShift', () => {
    ...
    describe('night shift filtering (cross-day)', () => {
      ...
      it('should include next-day admissions without time as part of night shift', () => {
        // In production some admissions are stored only with date (no HH:MM)
        // For Jan 3rd night shift, Jan 4th admissions without time should still appear
        expect(isAdmittedDuringShift(recordDate, nextDay, undefined, 'night')).toBe(true);
      });
