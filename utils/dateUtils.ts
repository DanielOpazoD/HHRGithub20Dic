...
export const isAdmittedDuringShift = (
  ...
  const admissionHour = admissionTime ? parseInt(admissionTime.split(':')[0], 10) : null;
  const hasValidAdmissionHour = admissionHour !== null && !isNaN(admissionHour);
  const isEarlyMorning = hasValidAdmissionHour && admissionHour >= 0 && admissionHour < 8;
  ...
  if (shift === 'day') {
    ...
    // If admissionTime is missing on next-day admissions, treat it as early morning
    // to keep cross-day night-shift ownership (20:00 -> 08:00).
    if (patientAdmissionDate === nextDay && (!isDayTime || !admissionTime)) {
